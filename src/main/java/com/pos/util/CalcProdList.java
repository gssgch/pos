...
public class CalcProdList {
    ...
    public static ArrayList<ShopDetailVO> queryShopMap(String regionStr, String id, String amount) {
        ...
        ArrayList<ShopDetailVO> shopList = new ArrayList();
        if (lis.size() > 0) {
            ...
            for (int i = 0; i < lis.size(); i++) {
                ...
                vo1.setUnitPrice(new BigDecimal(
                        (String) tempMap.get("DETAILS_PRICE")));
    ...
    public static void createWordByMap(Map map1, String regionStr)
        ...
        if (map1.size() > 1) {
            ...
            ArrayList<ShopDetailVO> lis = null;
            Map shopMap = null;
            Set<String> ss = dateMap.keySet();
            ...
            for (String s : ss) {
                ...
                String shopDataId = (String) shopMap.get("shopDataId");
                String shopCardId = (String) shopMap.get("shopCardId");
                System.out.println("===shopName=" + shopName + "--typeName=" + typeName +
                        "--shopDataId=" + shopDataId + "--shopCardID=" + shopCardId);
                ...
                if (!"".equals(shopName)) {
                    ...
                    if ((lis == null) || (lis.size() == 0)) {
                        ...
                        for (String tt : ts1) {
                            ...
                            System.out.println("===shopDetailsName=" + shopDetailsName);
...
//                                            unitPrice, unitPriceSum, danwei));
                                    getStr(shopDataId, shopName, shopDetailsName, countStr,
                                            unitPrice, unitPriceSum, danwei));
...
//        else {
                ...
                cardMap.put("ushopDataId", shopDataId);
                cardMap.put("ushopCardId", shopCardId);
                ...
                System.out.println("==22222==inter new func getStrUpdate===");
...
//        }
     ...
     * 修改后方法
     * */
    public static String getStr(String shopDataId, String shopName, String shopDetailsName, int count, BigDecimal unitPrice, BigDecimal unitPriceSum, String danwei) {
        System.out.println("====inter new func getStrUpdate===");
        StringBuffer sb = new StringBuffer();
        if ("1".equals(shopDataId)) {
//    if (("常州明都酒店".equals(shopName)) || ("常州新城希尔顿酒店".equals(shopName)) || ("环洲绿岛咖啡延陵店".equals(shopName)) || ("六福珠宝(常州新世纪商城)".equals(shopName))) {
        ...
        } else if ("2".equals(shopDataId)) {
//      if("传奇人生音乐酒吧".equals(shopName)) {
            sb.append(shopDetailsName);
        ...
        } else if ("3".equals(shopDataId)) {
//      if ("大润发（常州店）".equals(shopName)) {
        ...
        } else if ("4".equals(shopDataId)) {
//      if ("海澜之家（关河东路店）".equals(shopName)) {
            sb.append(shopDetailsName);
            if (shopDetailsName.getBytes().length < 16) {
        ...
        } else if ("5".equals(shopDataId)) {
//      if ("常州泰富百货集团有限公司".equals(shopName)) {
            sb.append(shopDetailsName);
            if (shopDetailsName.getBytes().length < 12) {
        ...
        } else if ("6".equals(shopDataId)) {
//      if ("家乐福(乐家邹区店)".equals(shopName)) {
            if (shopDetailsName.getBytes().length < 12) {
                sb.append(getStr(10 - shopDetailsName.getBytes().length));
            } else {
                sb.append(" ");
            ...
            do {
                ...
                for (String tt : ts1) {
                    ...
                    if (tempBig.compareTo(new BigDecimal("0")) > 0) {
                        if (dd.getUnitPriceSrc().divide(new BigDecimal(4)).compareTo(tempBig) >= 0) {

                            if (tempBig.multiply(new BigDecimal(dd.getCount())).compareTo(minusCount) <= 0) {
    ...
    public static Map createProductList(ArrayList<ShopDetailVO> lis, BigDecimal amount, BigDecimal minusCount, int productCount, String keyStr) {
        ArrayList<ShopDetailVO> newList = null;
