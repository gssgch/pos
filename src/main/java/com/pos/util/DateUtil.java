...
public class DateUtil extends JPanel
{
    ...
    public void register(JComponent showComponent) {
        this.showDate = showComponent;
        showComponent.setRequestFocusEnabled(true);
        showComponent.addMouseListener(new MouseAdapter(showComponent) {
            public void mousePressed(MouseEvent me) {
                this.val$showComponent.requestFocusInWindow();
            }
        });
        ...
        showComponent.addMouseListener(new MouseAdapter(showComponent) {
            public void mouseEntered(MouseEvent me) {
                if (this.val$showComponent.isEnabled())
                    this.val$showComponent.setCursor(new Cursor(12));
            }

            public void mouseExited(MouseEvent me)
            {
                if (this.val$showComponent.isEnabled()) {
                    this.val$showComponent.setCursor(new Cursor(0));
                    this.val$showComponent.setForeground(Color.BLACK);
                }
            }

            public void mousePressed(MouseEvent me) {
                if (this.val$showComponent.isEnabled()) {
                    this.val$showComponent.setForeground(DateUtil.hoverColor);
                    if (DateUtil.this.isShow)
                        DateUtil.this.hidePanel();
                    else
                        DateUtil.this.showPanel(this.val$showComponent);
                }
            }
    ...
    public static String getDateStr(String paramDateStr, String flag, String shopName)
    {
        ...
        try {
            ...
            break label273: if ("传奇人生音乐酒吧".equals(shopName))
                sdf.applyPattern("yyyy/MM/dd HH:mm:ss");
            try {
                Date d = sdf.parse(paramDateStr);
                sdf.applyPattern("yyyy-MM-dd   HH:mm:ss");
                ...
                break label273: if ("大润发（常州店）".equals(shopName)) {
                    sdf.applyPattern("yyyy/MM/dd HH:mm:ss");
                    try {
                        Date d = sdf.parse(paramDateStr);
                        ...
                        break label273: if (("2".equals(flag)) &&
    ...
    private class BodyPanel extends JPanel
    {
        ...
        public void updateDate() {
            ...
            for (DateUtil.WeekLabel weekLabel : DateUtil.weekLabels) {
                add(weekLabel);
            }
            for (int i = 0; i < 42; ++i) {
                cal.add(5, 1);
                DateUtil.this.lm.addLabel(new DateUtil.DayLabel(DateUtil.this, cal));
            }
    ...
    private class DayLabel extends JLabel
            implements Comparator<DayLabel>, MouseListener, MouseMotionListener
    {
    ...
    private class TitlePanel extends JPanel
    {
        ...
        private void initTitlePanel() {
            ...
            this.preYear.addMouseListener(
                    new MyMouseAdapter(this.preYear, 1,
                            -1));
            this.preMonth.addMouseListener(
                    new MyMouseAdapter(this.preMonth,
                            2, -1));
            this.nextMonth.addMouseListener(
                    new MyMouseAdapter(this.nextMonth,
                            2, 1));
            this.nextYear.addMouseListener(
                    new MyMouseAdapter(this.nextYear,
                            1, 1));
        }
 ...
 * Qualified Name:     com.pos.util.DateUtil
